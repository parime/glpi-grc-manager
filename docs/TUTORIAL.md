...
probabilité x impact, la Déclaration d'Applicabilité (SoA) et le programme d'audit interne avec
ses actions correctives et préventives (CAPA). Toutes les captures ci-dessous viennent d'une
instance réelle, avec de vraies données (exemples anonymisés).
...
Le plugin ajoute ses quatre écrans (Risques, Contrôles Annexe A, Audits internes,
Non-conformités) dans le menu **Outils**, à côté du registre de risques fournisseurs.
...
Statement of Applicability (SoA) and the internal audit program with its corrective and
preventive actions (CAPA). Every screenshot below comes from a real instance, with real data
(anonymized examples).
...
The plugin adds its four screens (Risks, Annex A Controls, Internal audits, Non-conformities) to
the **Outils** menu, next to the supplier risk register.
